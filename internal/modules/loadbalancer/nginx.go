...
// 实现约定:WriteConfig 后由调用方先 Test 通过才 Reload;Test 失败不得 Reload。
type Nginx interface {
	// WriteConfig 把均衡组配置写到 confDir/lb_<name>.conf(原子写)。
	WriteConfig(name, content string) error
	// RemoveConfig 删除 confDir/lb_<name>.conf。文件不存在视为成功。
...
// confFileName 是均衡组配置文件名(带 lb_ 前缀,避免与其它模块冲突)。
func confFileName(name string) string { return "lb_" + name + ".conf" }
...
func (n *realNginx) confPath(name string) (string, error) {
	...
	p := filepath.Join(n.confDir, confFileName(name))
...
func (n *realNginx) WriteConfig(name, content string) error {
	...
	tmp, err := os.CreateTemp(n.confDir, confFileName(name)+".*.tmp")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
